...
export const Header: React.FC<HeaderProps> = ({ onBookSession }) => {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 glass-header text-white transition-all duration-300">
      ...
      <div
        className="absolute top-0 left-0 h-0.5 bg-accent-gold transition-all duration-150 ease-out"
      ...
      <div className="max-w-7xl mx-auto px-6 sm:px-10 md:px-14 h-20 flex items-center justify-between">
        ...
        <div className="flex items-center">
          <a href="#craft" className="flex items-center gap-3 group">
            <span className="font-sans font-bold text-[16px] sm:text-[17px] tracking-[0.08em] uppercase text-white group-hover:text-accent-gold transition-colors">
              Duane Henry
            </span>
          ...
          <div className="flex flex-col space-y-4">
            {navLinks.map((link) => (
              <a
                ...
                className="text-[14px] font-medium tracking-[0.12em] uppercase text-zinc-300 hover:text-accent-gold py-2 border-b border-white/5 transition-colors"
              >
                {link.label}
              </a>
            ))}
