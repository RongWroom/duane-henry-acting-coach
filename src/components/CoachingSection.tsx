...
export const CoachingSection: React.FC<CoachingSectionProps> = ({ onSelectModule }) => {
  const { sectionNumber, sectionTitle, subtitle, category, description, sessionLocations, modules } =
    COACHING_DATA;
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-start">
          ...
          <div className="lg:col-span-5 flex flex-col space-y-8">
            <ScrollReveal delay={0.1}>
              ...
              <p className="text-[16px] sm:text-[17px] leading-[1.75] font-normal text-zinc-300">
                {description}
              </p>
            </ScrollReveal>
                ...
                <ScrollReveal key={item.id} delay={0.1 + idx * 0.08}>
                  ...
                  >
                    ...
                    <button
                      ...
                      className="w-full p-6 sm:p-7 flex items-center justify-between text-left group gap-4 cursor-pointer"
                    ...
                    {/* Expanded Content */}
                    {isOpen && (
                      <div className="px-6 sm:px-7 pb-7 pt-2 border-t border-white/10 animate-in fade-in-50 duration-200">
                        ...
                        {item.focusAreas && (
                          <div className="mt-5 flex flex-wrap items-center gap-2">
                            ...
                            {item.focusAreas.map((tag) => (
                              <span
                                key={tag}
                                className="px-3 py-1 rounded-full bg-white/5 text-zinc-300 text-[11px] font-medium border border-white/10"
                              >
                                {tag}
                              </span>
                            ))}
