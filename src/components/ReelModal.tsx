...
export const ReelModal: React.FC<ReelModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[10000] flex items-center justify-center p-4 sm:p-6 md:p-10 bg-black/90 backdrop-blur-md animate-in fade-in duration-200">
      <div className="relative w-full max-w-5xl bg-[#1A1715] border border-[#DDD5C7]/30 shadow-2xl overflow-hidden flex flex-col">
        {/* Header Bar */}
        <div className="p-4 sm:p-5 flex items-center justify-between border-b border-white/10 bg-black/40">
          <div className="flex items-center gap-3">
            <span className="w-2.5 h-2.5 rounded-full bg-red-600 animate-pulse" />
            <span className="font-sans text-[11px] font-semibold tracking-[0.2em] uppercase text-[#FDFCF7]">
          ...
          <button
            ...
            className="w-8 h-8 flex items-center justify-center text-[#DDD5C7] hover:text-white transition-colors"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="relative aspect-[16/9] w-full bg-black flex items-center justify-center overflow-hidden group">
          ...
          <button
            ...
            className="absolute z-20 w-20 h-20 rounded-full bg-black/60 border border-white/20 text-[#FDFCF7] flex items-center justify-center hover:scale-110 transition-transform shadow-2xl backdrop-blur-sm"
          >
            {isPlaying ? (
          ...
          <div className="absolute bottom-6 left-6 right-6 flex items-center justify-between text-[11px] text-white/90 bg-black/70 backdrop-blur-sm px-4 py-2 border border-white/10">
            ...
            <span className="font-mono text-[11px] text-[#DDD5C7]">
        ...
        <div className="p-4 sm:p-6 bg-[#1A1715] text-[#DDD5C7]">
          {/* Progress Scrubber */}
          <div className="relative w-full h-1.5 bg-white/20 mb-6 cursor-pointer group">
            <div
              className="h-full bg-[#DDD5C7] relative"
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-4">
              ...
              <span className="text-[12px] font-mono text-[#8C8275]">
                {formatTime(currentTime)} / {formatTime(totalDuration)}
              </span>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-2 text-[10px] tracking-wider uppercase">
              <span className="text-[#8C8275]">Jump Scene:</span>
              {scenes.map((s, idx) => (
                <button
                  key={idx}
                  onClick={() => setCurrentTime(idx * 60 + 15)}
                  className="px-2.5 py-1 border border-white/15 hover:border-[#DDD5C7] text-[#DDD5C7] hover:text-white transition-colors"
                >
                  {s.role} ({s.time})
                </button>
              ))}
