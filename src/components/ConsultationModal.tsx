...
export const ConsultationModal: React.FC<ConsultationModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-10000 flex items-center justify-center p-4 sm:p-6 bg-black/85 backdrop-blur-md animate-in fade-in duration-200">
      <div className="relative w-full max-w-2xl bg-page-bg-alt border border-white/15 rounded-2xl shadow-2xl p-6 sm:p-10 max-h-[90vh] overflow-y-auto text-white">
        <button
          onClick={onClose}
        ...
        {!submitted ? (
          <form onSubmit={handleSubmit} className="space-y-6">
            <div>
              <div className="text-[11px] font-semibold tracking-[0.2em] uppercase text-accent-gold mb-1.5">
                Direct Inquiries
              </div>
            ...
            <div className="space-y-5">
              <div>
                ...
                <input
                  ...
                  className="w-full bg-input-bg border border-white/10 rounded-lg px-4 py-3 text-[15px] text-white focus:outline-none transition-colors"
                />
              </div>
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full bg-input-bg border border-white/10 rounded-lg px-4 py-3 text-[15px] text-white focus:outline-none transition-colors cursor-pointer"
                >
                  {COACHING_DATA.modules.map((m) => (
                    <option key={m.id} value={m.title} className="bg-input-bg">
                      {m.title}
                    </option>
                  ))}
                  <option value="General Consultation" className="bg-input-bg">
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full bg-input-bg border border-white/10 rounded-lg px-4 py-3 text-[15px] text-white focus:outline-none transition-colors resize-none"
                />
              </div>
            </div>
            ...
            <div className="pt-4 flex flex-col sm:flex-row items-center justify-between gap-4 border-t border-white/10">
              ...
              <button
                ...
                className="w-full sm:w-auto px-7 py-3 rounded-full bg-white text-black hover:bg-accent-gold text-[12px] font-bold tracking-[0.14em] uppercase transition-all duration-200 flex items-center justify-center gap-2"
              >
                {isSubmitting ? (
                  <>
          ...
          <div className="py-8 text-center space-y-5 animate-in fade-in duration-300">
            ...
            <p className="text-[15px] text-zinc-300 max-w-sm mx-auto leading-relaxed">
              Your session inquiry for{' '}
              <strong className="text-white">{objective}</strong> has been received. Duane will review your materials and contact you at <strong className="text-white">{email}</strong>.
            </p>
            <div className="pt-2">
              <button
                onClick={onClose}
                className="px-6 py-2.5 rounded-full bg-white text-black hover:bg-accent-gold text-[11px] font-bold tracking-[0.14em] uppercase transition-colors"
              >
                Close
              </button>
            </div>
